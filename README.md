...
| 数値変換 | `convert<T>(x)` / `T(x)`で値を保持、`trunc<T>(x)`で小数部分を切り捨て |
...
**計算の規則：** 暗黙の数値変換はありません。整数の桁あふれ、不正な整数除算、範囲外参照、正確な変換で値を保てない場合や、切り捨て後に範囲を超える場合は停止します。浮動小数点計算には丸めがあります。

**未実装：** 再帰、動的配列、文字列の連結・添字参照、実行時停止の捕捉、四捨五入などの丸め・飽和変換。
