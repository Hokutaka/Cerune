...
fn function_arguments_keep_order_nested_calls_and_copies() {
    assert_eq!(
        run_vm(include_str!("../examples/function_arguments.ceru")).unwrap(),
...
fn packet_counter_keeps_high_bits_and_independent_copies() {
    assert_eq!(
        run_vm(include_str!("../examples/packet_counter.ceru")).unwrap(),
        "packet counter\n9223372036854775808\n9223372036854775809\n1\n3\ntrue\n9\ntrue\n受信\0\r\n\n"
    );
}
...
fn native_values_keep_mixed_arguments_and_aggregate_copies() {
    assert_eq!(
        run_vm(include_str!("../examples/native_values.ceru")).unwrap(),
        "native values\n18446744073709551615\n-7\n4\n4\n18446744073709551615\n9223372036854775808\n日本語\0\n"
    );
}
...
fn string_values_preserve_copies_unicode_and_line_breaks() {
    assert_eq!(
        run_vm(include_str!("../examples/string_values.ceru")).unwrap(),
        "こんにちは\nこんばんは\ntrue\ntrue\n一行目\n二行目\n"
    );
}
...
fn string_lookup_searches_product_entries_and_uses_defaults() {
    assert_eq!(
        run_vm(include_str!("../examples/string_lookup.ceru")).unwrap(),
        "こんにちは\nさようなら\n未登録\n見つかりません\n"
    );
}
...
fn small_values_remain_visible_while_arithmetic_can_round() {
    let output = run_vm(include_str!("../examples/small_values.ceru")).unwrap();
...
fn measurement_statistics_preserves_fractional_mean_and_variance() {
    assert_eq!(
        run_vm(include_str!("../examples/measurement_statistics.ceru")).unwrap(),
        "12.5\n5.25\n12.5\n5.25\n"
    );
}
...
fn normalized_histogram_preserves_probabilities_and_recovers_counts() {
    assert_eq!(
        run_vm(include_str!("../examples/normalized_histogram.ceru")).unwrap(),
        "0.125\n0.25\n0.375\n0.25\n1\n1\n2\n3\n2\n"
    );
}
...
fn bit_flags_keep_switches_independent() {
    assert_eq!(
        run_vm(include_str!("../examples/bit_flags.ceru")).unwrap(),
        "3\ntrue\n1\n5\n2\n255\n"
    );
}
...
fn ring_buffer_keeps_the_latest_four_samples_in_order() {
    assert_eq!(
        run_vm(include_str!("../examples/ring_buffer.ceru")).unwrap(),
        "10\n15\n20\n25\n35\n45\n30\n40\n50\n60\n"
    );
}
...
fn subset_sum_bits_records_reachable_totals() {
    assert_eq!(
        run_vm(include_str!("../examples/subset_sum_bits.ceru")).unwrap(),
        "1\n9\n297\n19305\ntrue\ntrue\nfalse\ntrue\n"
    );
}
...
fn color_blending_widens_before_adding_channels() {
    assert_eq!(
        run_vm(include_str!("../examples/color_blending.ceru")).unwrap(),
        "160\n120\n140\n"
    );
}
...
fn sensor_calibration_preserves_signed_small_values() {
    assert_eq!(
        run_vm(include_str!("../examples/sensor_calibration.ceru")).unwrap(),
        "-32003\n37\n31997\n10\n"
    );
}
...
fn short_circuit_guards_division_and_array_access() {
    assert_eq!(
        run_vm(include_str!("../examples/short_circuit.ceru")).unwrap(),
        "false\n1\n3\ntrue\ntrue\n99\ntrue\n"
    );
}
...
fn maximum_subarray_tracks_the_best_contiguous_sum() {
    assert_eq!(
        run_vm(include_str!("../examples/maximum_subarray.ceru")).unwrap(),
        "1\n-2\n4\n3\n5\n6\n1\n5\n6\n"
    );
}
...
fn population_statistics_widens_the_sum_and_preserves_unsigned_ordering() {
    assert_eq!(
        run_vm(include_str!("../examples/population_statistics.ceru")).unwrap(),
        "3100000000\n3400000000\n"
    );
}
...
fn integer_conversions_example_preserves_both_results_and_the_original() {
    let output = run_vm(include_str!("../examples/integer_conversions.ceru")).unwrap();
...
fn hello_example_adds_two_named_integers() {
    let output = run_vm(include_str!("../examples/hello.ceru")).unwrap();
...
fn floating_point_example_exposes_precision_and_inferred_type() {
    let output = run_vm(include_str!("../examples/floating_point.ceru")).unwrap();
...
fn coin_change_finds_a_better_answer_than_greedy_selection() {
    let output = run_vm(include_str!("../examples/coin_change.ceru")).unwrap();
...
fn shortest_paths_preserves_unreachable_pairs_and_finds_indirect_routes() {
    let output = run_vm(include_str!("../examples/shortest_paths.ceru")).unwrap();
...
fn heat_diffusion_updates_from_one_time_step_and_preserves_initial_values() {
    let output = run_vm(include_str!("../examples/heat_diffusion.ceru")).unwrap();
...
fn linear_regression_learns_the_line_and_reduces_loss() {
    let output = run_vm(include_str!("../examples/linear_regression.ceru")).unwrap();
    let values: Vec<f64> = output.lines().map(|line| line.parse().unwrap()).collect();
...
fn integer_limits_example_runs_without_overflow() {
    let output = run_vm(include_str!("../examples/integer_limits.ceru")).unwrap();
...
fn square_root_example_runs() {
    let output = run_vm(include_str!("../examples/square_root.ceru")).unwrap();
...
fn logistic_map_example_exposes_f32_and_f64_divergence() {
    let output = run_vm(include_str!("../examples/logistic_map.ceru")).unwrap();
...
fn boolean_comparisons_example_runs() {
    let output = run_vm(include_str!("../examples/boolean_comparisons.ceru")).unwrap();
...
fn conditional_example_runs() {
    let output = run_vm(include_str!("../examples/conditional.ceru")).unwrap();
...
fn while_square_root_example_runs() {
    let output = run_vm(include_str!("../examples/while_square_root.ceru")).unwrap();
...
fn loop_control_example_runs() {
    let output = run_vm(include_str!("../examples/loop_control.ceru")).unwrap();
...
fn for_sum_example_runs() {
    let output = run_vm(include_str!("../examples/for_sum.ceru")).unwrap();
...
fn euclidean_gcd_example_runs() {
    let output = run_vm(include_str!("../examples/euclidean_gcd.ceru")).unwrap();
...
fn fibonacci_example_runs() {
    let output = run_vm(include_str!("../examples/fibonacci.ceru")).unwrap();
...
fn factorial_example_runs() {
    let output = run_vm(include_str!("../examples/factorial.ceru")).unwrap();
...
fn collatz_example_runs() {
    let output = run_vm(include_str!("../examples/collatz.ceru")).unwrap();
...
fn prime_check_example_runs() {
    let output = run_vm(include_str!("../examples/prime_check.ceru")).unwrap();
...
fn integer_square_root_example_runs() {
    let output = run_vm(include_str!("../examples/integer_square_root.ceru")).unwrap();
...
fn exponentiation_by_squaring_example_runs() {
    let output = run_vm(include_str!("../examples/exponentiation_by_squaring.ceru")).unwrap();
...
fn pythagorean_triples_example_runs() {
    let output = run_vm(include_str!("../examples/pythagorean_triples.ceru")).unwrap();
...
fn product_point_example_runs() {
    let output = run_vm(include_str!("../examples/product-point.ceru")).unwrap();
...
fn functions_example_runs() {
    let output = run_vm(include_str!("../examples/functions.ceru")).unwrap();
...
fn function_values_example_runs() {
    let output = run_vm(include_str!("../examples/function_values.ceru")).unwrap();
...
fn fixed_arrays_example_runs() {
    let output = run_vm(include_str!("../examples/fixed_arrays.ceru")).unwrap();
...
fn bubble_sort_example_runs() {
    let output = run_vm(include_str!("../examples/bubble_sort.ceru")).unwrap();
...
fn product_arrays_example_runs() {
    let output = run_vm(include_str!("../examples/product_arrays.ceru")).unwrap();
...
fn matrix_vector_product_example_runs() {
    let output = run_vm(include_str!("../examples/matrix_vector_product.ceru")).unwrap();
...
fn matrix_composition_example_runs() {
    let output = run_vm(include_str!("../examples/matrix_composition.ceru")).unwrap();
...
fn xor_neural_network_example_runs() {
    let output = run_vm(include_str!("../examples/xor_neural_network.ceru")).unwrap();
