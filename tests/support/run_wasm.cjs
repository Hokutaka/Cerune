...
const imports = { cerune: {
  ...
  // 比較fixtureは正確に表せる1.5、2、2.5、4を表示します。汎用の数値整形ホストではありません。
  print_f32: value => { if (value !== 1.5 && value !== 2 && value !== 2.5 && value !== 4) throw new Error('unsupported test float'); writeText(value); },
  print_f64: value => { if (value !== 1.5 && value !== 2 && value !== 2.5 && value !== 4) throw new Error('unsupported test float'); writeText(value); },
} };
