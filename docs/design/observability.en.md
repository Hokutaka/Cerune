...
This document defines the boundaries that Cerune's observation features must preserve.

Cerune prioritizes the ability to observe the transformation process from source code to generated artifacts. The ability to observe compilation is distinct from the ability to interfere with it.

## Separation of observation and control

- Observation APIs are read-only.
- Cerune does not provide an API that allows external users to modify compiler state directly.
...
- Cerune preserves enough provenance to trace elements across tokens, the AST, semantic-analysis results, and IR.
- Every Cerune IR statement and expression has a deterministic `NodeId` unique within one compilation. `NodeId` identity is distinct from source-location `Span` data.
- Source ranges are preserved for syntax elements, and each bytecode instruction records its originating Cerune IR `NodeId` and diagnostic `Span`, or that it was generated by the compiler.
...
- The same Cerune version, source, output route, target, target features, and explicit options produce deterministic observation results.
- `NodeId` values are deterministic under those conditions, but they are not persistent IDs across source edits or Cerune versions.
...
`emit-llvm --annotate-origins` adds read-only comments to the existing LLVM artifact. The header identifies the format as `; cerune-origins v1`. A comment such as `; cerune-origin: #7 bytes 208..235` connects the following instruction group to node #7 in `emit-ir`. These offsets identify `echo(left) == "日本語\0"` in the LF version of `examples/string_origins.ceru`. Ranges are UTF-8 byte offsets into the original input, with an exclusive end; CRLF input changes offsets.
