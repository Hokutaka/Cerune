...
2026-09-20時点、モジュール・引数制限の解消・構造体の更新式・コンパイル時定数・直和型とmatchまでを反映しています。「現在」は実装済み、「候補」は設計・実装前の提案です。候補の構文や採用を確定する文書ではありません。現在の正確な仕様は[言語リファレンス](../reference/language.ja.md)を参照してください。
...
| 2（基礎実装済み） | コードを分ける仕組み | モジュール・名前空間・公開範囲を実装。コンパイル時定数を共有できる |
| 2 | 関数・値操作の実用上の制限を減らす | 4引数制限は全経路で解消済み。構造体の更新式で一部を変えた新しい値を作れる。混在引数とコピー後の独立性を検証する |
| 3（基礎実装済み） | 選択肢と回復可能な失敗を値で表す仕組み | `enum`と網羅的な`match`を実装。存在/不在を値で返す。ジェネリクスや暗黙のエラー伝播は未実装 |
...
[ファイルを識別できるソース位置](source-files.ja.md)に続き、[モジュール](modules.ja.md)の明示的なimport・名前空間・関数と型の公開範囲を実装しました。CLIから分割版と単一ファイル版を全経路で実行比較します。[コンパイル時定数](constants.ja.md)も実装済みです。再exportとパッケージ配布は未実装です。関数の4引数制限も解消し、[混在引数の例](../../examples/function_arguments.ceru)で評価順とコピーを比較しています。[構造体の更新式](product-updates.ja.md)も実装し、コピー・評価順・停止を全経路で比較します。[直和型と網羅的な分岐](sum-types.ja.md)を実装し、存在/不在とペイロードのコピーを全経路で比較します。次は配列長の取得や反復など、配列・数値処理の再利用を具体例から整理します。
