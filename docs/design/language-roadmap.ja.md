...
2026-09-19時点、モジュール・引数制限の解消・構造体の更新式・コンパイル時定数までを反映しています。「現在」は実装済み、「候補」は設計・実装前の提案です。候補の構文や採用を確定する文書ではありません。現在の正確な仕様は[言語リファレンス](../reference/language.ja.md)を参照してください。
...
[ファイルを識別できるソース位置](source-files.ja.md)に続き、[モジュール](modules.ja.md)の明示的なimport・名前空間・関数と型の公開範囲を実装しました。CLIから分割版と単一ファイル版を全経路で実行比較します。[コンパイル時定数](constants.ja.md)も実装済みです。再exportとパッケージ配布は未実装です。関数の4引数制限も解消し、[混在引数の例](../../examples/function_arguments.ceru)で評価順とコピーを比較しています。[構造体の更新式](product-updates.ja.md)も実装し、コピー・評価順・停止を全経路で比較します。次は「存在する／しない」「成功／失敗」を値で扱う直和型と分岐を、具体例から検討します。
