...
Constants and functions with explicit type arguments follow the same rules. Evaluation order, short-circuiting, and independent array copies are preserved. Other rounding policies, saturation, and integer bit truncation remain separate proposals.

## Representation and emission

`ConversionMode::{Exact, Truncate}` is retained from the AST, separately from source spelling in `ConversionSyntax`. IR and bytecode show `convert.exact` or `convert.trunc`. Each backend receives the types and mode without reinterpreting source syntax.
